use crate::config::{ ConfigJSON, CacheValue };
...
pub fn register_config_file() -> anyhow::Result<ConfigJSON> {
  ...
  println!("Creating configuration file with default values at {}", format!("{:?}", config_path).cyan());
...
pub fn save_api_key(api_key: &str) -> anyhow::Result<()> {
  ...
  println!("{}", format!("Saved {} API key successfully!", "encrypted".cyan()).green());
  Ok(())
}
