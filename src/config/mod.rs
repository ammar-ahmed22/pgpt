...
impl ConfigSetters {
  pub fn set(&self) -> anyhow::Result<()> {
    ...
    match self {
      Self::APIKey { value } => {
        match value.trim() {
          ...
          e => {
            return Err(anyhow::anyhow!("Received empty API key - {}", e))
          } 
        }
      },
      Self::CacheLength { value } => {
        println!("Setting {} to {}", "cache-length".cyan(), value.to_string().cyan());
    ...
    let mut file = std::fs::File::create(&config_path).with_context(|| format!("Could not open file"))?;
    let config_str = serde_json::to_string(&config)?;
    file.write(&config_str.as_bytes()).with_context(|| format!("Could not write to file"))?;
...
impl ConfigSettings {
  ...
  pub fn show(&self) -> anyhow::Result<()> {
    let config = utils::load_config_file().unwrap_or_else(|_| utils::register_config_file().unwrap_or_else(|e| panic!("{:#?}", e)));
    let api_key = utils::load_api_key().unwrap_or_else(|_| utils::register_api_key().unwrap_or_else(|e| panic!("{:#?}", e)));
...
impl Config {
  ...
  pub fn load_config() -> anyhow::Result<Arc<Self>>{
    let api_key = utils::load_api_key().unwrap_or_else(|_| utils::register_api_key().unwrap_or_else(|e| panic!("{:#?}", e)));
    let config_json = utils::load_config_file().unwrap_or_else(|_| utils::register_config_file().unwrap_or_else(|e| panic!("{:#?}", e)));
    ...
    let config = Self { api_key, model, cache_length: config_json.cache_length };
    Ok(Arc::new(config))
  }
  ...
  pub fn parse_args() -> ParsedArgs {
    let cli = CLI::parse();
    match cli.command {
      Commands::Query { query, cost, model, context } => {
        ...
        ParsedArgs::Query { args: Arc::new(args) }
      },
      Commands::Config { config_commands } => ParsedArgs::Config { config: config_commands }
    }
  }
  ...
  pub fn handle_config(config_commands: &ConfigCommands) -> anyhow::Result<()> {
    match config_commands {
      ConfigCommands::Show { config_settings } => {
        config_settings.show()
      },
      ConfigCommands::Set { config_setters } => {
        config_setters.set()
      },
      ConfigCommands::Clear { config_removers } => {
        config_removers.clear()
      }
    }
    // Ok(())
  }
